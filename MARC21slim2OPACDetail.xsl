  ...
  exclude-result-prefixes="marc items">
            ...
            <!--<xsl:if test="marc:datafield[@tag=246]">
                <h1 class="title" property="name">=
                    <xsl:for-each select="marc:datafield[@tag=246]">
                        ...
                        <xsl:text> </xsl:text>
                        <xsl:for-each select="marc:subfield[contains('b', @code)]">
                            <xsl:choose>
                                <xsl:when test="@code='h'">
                                    <span class="title_medium"><xsl:apply-templates/> </span>
                                </xsl:when>
   ...
   <!--<xsl:if test="$DisplayOPACiconsXSLT!='0'">
        ...
        <xsl:if test="marc:datafield[@tag=942]/marc:subfield[@code='c']">
          <span class="results_summary type"><span class="label">Item type: </span>
            <xsl:for-each select="marc:datafield[@tag=942]/marc:subfield[@code='c']">
              <xsl:value-of select="." />
              <xsl:choose>
        ...
        <xsl:if test="marc:datafield[@tag=026]">
          <span class="results_summary"><span class="label">Fingerprint: </span>
              <xsl:value-of select="marc:datafield[@tag=026]/marc:subfield[@code='a']" />
          </span>
        </xsl:if>
        ...
        <!-- To do: Tobias: ad rule that creates punctuation (test if count > 1 then ";" etc.)-->
        <xsl:choose>
...
<!--Tobias: Only show first ISBN (due to double ISBN's) and no dot (.) at the end -->
<xsl:if test="marc:datafield[@tag=020]/marc:subfield[@code='a']">
  <span class="results_summary isbn"><span class="label">ISBN: </span>
    <xsl:for-each select="marc:datafield[@tag=020][1]/marc:subfield[@code='a'][1]">
      <span property="isbn">
        <xsl:value-of select="."/>
      </span>
    </xsl:for-each>
                    ...
                    <xsl:for-each select="marc:datafield[@tag=246]">
                        <span property="alternateName">
                            <xsl:call-template name="chopPunctuation">
                                <xsl:with-param name="chopString">
                                    <xsl:call-template name="subfieldSelect">
<!--modified by Tobias 'abhfgnp' to 'abhfnp' -->
                                        <xsl:with-param name="codes">abhfgnp</xsl:with-param>
                                    </xsl:call-template>
                                </xsl:with-param>
                            </xsl:call-template>
...
<!--Tobias: don't show 653 fields?, in results (does this count for facets as well?) and details -->
            <xsl:if test="marc:datafield[substring(@tag, 1, 1) = '6' and not(@tag=655) and not(@tag=653)]">
            <span class="results_summary subjects"><span class="label">Subject(s): </span>
                <xsl:for-each select="marc:datafield[substring(@tag, 1, 1) = '6'][not(@tag=655) and not(@tag=653)]">
...
<!-- Image processing code added here, takes precedence over text links including y3z text   -->
        ...
        <!-- 773 -->
        <xsl:if test="marc:datafield[@tag=773 or @tag=774]">
        <xsl:for-each select="marc:datafield[@tag=774] | marc:datafield[@tag=773]">
        <!--<xsl:if test="@ind1 !=1">-->
        ...
        <xsl:text>: </xsl:text>
                ...
                <xsl:variable name="f773">
                    <xsl:call-template name="chopPunctuation"><xsl:with-param name="chopString"><xsl:call-template name="subfieldSelect">
                        <xsl:with-param name="codes">agw40</xsl:with-param>
<!--   original                 <xsl:with-param name="codes">a_t</xsl:with-param>-->
...
<!--copied code for 502 tag -->
        <xsl:if test="marc:datafield[@tag=518]">
            <span class="results_summary event_note">
                <span class="label">Event note: </span>
