...
<!-- dit werkt precies om te testen of er meer dan vijf 773 velden zijn en geeft de eerste vijf resultaten
nu nog scenario bij minder dan vijf (choose?)
en "..." (aanklikbaar) toevoegen als 6e resultaat -->
<!-- kan dit weg?  precies niet -->
<!--        <xsl:sort select="result"/> -->
        <xsl:if test="position() &lt; 6">
        <xsl:if test="marc:subfield[@code='t'] or marc:subfield[@code='a'] ">
...
<xsl:text> </xsl:text>
    ...
    <!-- show collation information | To do: create class 'results_extent'-->
        <xsl:if test="marc:datafield[@tag=300]">
            <span class="results_summary publisher"><span class="label">Extent: </span>
                <xsl:for-each select="marc:datafield[@tag=300]">
                    <xsl:if test="marc:subfield[@code='a']">
                        <xsl:call-template name="subfieldSelect">
                            <xsl:with-param name="codes">a</xsl:with-param>
				...
				<xsl:otherwise>
				  <xsl:attribute name="href"><xsl:value-of select="marc:subfield[@code='u']"/></xsl:attribute>
                   ...
                   <span class="available">
                       <b><xsl:text>Items available for consultation: </xsl:text></b>
